...
def is_fluff(text, sender, own_staff=None, is_robot=False):
    ...
    if is_robot:
        return False
    # 己方客服（昵称含统一前缀，如“亿民盛世-”）发的任何消息都不路由，从源头剔除
    if sender and own_staff:
        from .router import is_own_staff
        if is_own_staff(sender, own_staff):
            return True
    t = str(text or '').strip()
    if not t:
        return True
...
- A：中通"<回写机器人名>"反馈拦截/退改失败、无法退回，需回传客服群并@发单人。以下任一均判 ["A"]：
  已签收/被签收/已取出签收/签收代收点/在代收点或驿站；进村件已投递村站、已完成转运派送约定、无法受理；
  未发起过退改且已取出签收；已入柜入库且“若收件人已收到将无法退回/建议等有退回物流再处理售后”；
  快件正在派送中且“联系收件人拒收/若收件人已收到将无法退回/建议等有退回物流再处理售后”；未到达指定退改地址。
- NONE：不需要转发。

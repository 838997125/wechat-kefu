...
【群/动作定义】
- B：需要中通快递处理的快递操作（拦截、召回、签收未收到、催件、催派、送错地址核实等）
- C：催发货/需要尽快揽收发出
- D：仓库核实类（少件少发、提供发货视频/打包视频/称重图、特殊备注订单处理）
- A：中通"<回写机器人名>"反馈拦截失败（已签收/在代收点/进村件/未到达退改地址/无法受理），需要回传客服群并@发单人
- NONE：不需要转发（己方客服内部对话、水消息如"1/收到/好的"、发票、退货跟进话术、非指令闲聊、改地址等客服直接在快递群操作的）

【关键规则】
1. 发送人是己方客服（昵称含"<公司简称>""<己方客服D>""<己方客服E>""<己方客服B>""<己方客服C>""<己方客服F>""<己方客服H>""<己方客服G>"等）发的消息，是己方处理/搬运，输出 NONE。
2. 发送人"<回写机器人名>"：只有明确拦截失败（已签收/进村件/未到达退改地址/无法受理/拦截失败）才输出 ["A"]；
   正常"已通知网点/已受理/退回扫描已处理完成"输出 NONE。
3. 外部售后发的快递异常指令（拦截/召回/签收未收到/催件/送错地址）-> B。
4. 外部售后催发货（要快递尽快发出、催揽收）-> C。
5. 外部售后要核实少件/要发货视频/称重图/打包视频 -> D。
6. "退回中，已优先退款，请7日内跟进"这类是退货跟进话术，不是拦截指令 -> NONE。
7. 开发票、退件验收、备注送不送赠品等非快递操作 -> NONE。
8. 一条消息可同时命中多个（如少件既要B快递核实又要D仓库视频则 ["B","D"]），但多数只有一个。

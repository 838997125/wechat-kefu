...
DEFAULT_CONFIG = {
    "general": {
        ...
        # 数据保留：超过 N 天的消息/转发日志自动清理（0=永久保留），防止DB长期膨胀；面板默认只看近 message_view_days 天
        "retention_days": 90,
    ...
    "routing": {
        ...
        "quote_ack_text": "收到",
        # 己方客服识别：统一前缀方案——客服群昵称统一以“<公司简称>”开头，人员变动无需改这里。
        # 命中规则为“昵称包含任一关键词”，因此只需维护稳定的公司/团队标识；
        # 个别不带前缀的特殊内部账号，再往此列表补充关键词即可。
        "own_staff": ["<公司简称>", "。。"],
        ...
        "routes": [
            ...
            {
                "id": "rt_a2c",
                "name": "客服对接群 → <物流方>中通&<公司简称><公司简称>（催发货/催单，提取快递单号去重）",
                "enabled": True,
                "source": "<品牌A>客服对接群",
                "target": "<物流方>中通&<公司简称><公司简称>",
                "mode": "process",
            ...
            },
            {
                ...
                "target": "<品牌A>客服对接群",
                "mode": "process",
                # 只有“确认拦截失败/无法退回”的结果才回传：已签收/代收点/已取件/进村件；
                # 2026-09 客服确认以下三类也回流：进村件已投递村站无法受理、未发起退改已取出签收(代收点)、
                # 已入柜入库且提示若收件人已收到将无法退回。用精确词，不用泛化“无法退回”（成功受理条件句也含它）。
                # “快件正在派送中，联系收件人拒收/等退回物流”也回流（拦截进行中但可能已派送无法退回）。
                "keywords": ["已录签收", "已被签收", "已完成签收", "快件已签收", "已在代收点", "已在驿站",
                            "已被取件", "已取件", "已取出签收", "签收代收点", "拦截失败", "未到达指定退改地址",
                            "无法受理", "进村件", "投递至村站", "已完成转运派送", "已入柜入库",
                            "正在派送中", "联系收件人拒收"],
                "regex": "",
